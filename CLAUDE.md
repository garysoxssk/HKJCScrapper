...
**Resilience: nullable field audit + retry + response logging - COMPLETE**
- *Field fix*: `Match.featureStartTime`, `Match.featureMatchSequence`, `Tournament.frontEndId`, `FoPool.name_ch`/`name_en`/`expectedSuspendDateTime` relaxed to `Optional[str] = None`. `Match.frontEndId` stays strict (load-bearing: DB unique index, scheduler dedup keys).
- *Parser*: `parse_matches_response()` now iterates explicitly and logs+skips malformed individual matches instead of aborting the whole cycle. New `format_validation_errors()` helper renders `e.errors()` as `field.path: msg (got <type>)` lines.
- *Client*: replaced bare `print` with `logger`; new `_post_graphql()` helper centralizes timeout/connection/HTTP/JSON-decode logging. Reads `Settings.HKJC_REQUEST_TIMEOUT_SECONDS` instead of hardcoded 30.
- *Retry*: `compute_retry_budget()` + `call_with_retry()` in scheduler.py. Budget = `interval_seconds - 5` for continuous, fixed 120s for event/one-shot. `max_retries = min(cap, budget // (timeout + backoff))`. Retries on `requests.Timeout`/`ConnectionError`/`ChunkedEncodingError` only — HTTP 4xx/5xx and `ValidationError` fall through unchanged. Exponential backoff (1s, 2s, ...).
- *Scheduler*: split fetch+parse blocks in `run_discovery`/`execute_fetch`/`run_once` so failed parses log a truncated raw response body. `execute_fetch()` accepts `interval_seconds` kwarg (threaded through both live-schedule and reload paths) for budget computation.
- *TG*: `notify_error()` gained `details` kwarg and now includes exception type; cap raised 200→1000. New `_notify_error_with_details()` scheduler helper auto-formats `ValidationError.errors()` into the details body.
- *Config*: new `HKJC_REQUEST_TIMEOUT_SECONDS=30`, `HKJC_MAX_RETRIES=3`, `HKJC_RETRY_BACKOFF_SECONDS=1.0`.
- *Tests*: +15 (271 total). New: `test_match_model_accepts_null_feature_fields`, `test_match_model_strict_on_front_end_id`, `test_parse_matches_response_skips_malformed`, `test_format_validation_errors_includes_field_and_type`, `TestRetryBudget` (5), `TestCallWithRetry` (4), `test_notify_error_with_details`, `test_notify_error_includes_exception_type`.
- *Bugfix*: `MongoDBClient.__init__` previously force-set `tlsCAFile=certifi.where()` on every connection, which broke local plaintext mongod (SSL handshake against a non-TLS server). Made `tlsCAFile` conditional on `mongodb+srv://` scheme or `tls=true` in URI. Local smoke and `pytest -m mongodb` now work; Atlas connections unaffected.

**Round 2 TG UX enhancements - COMPLETE**
- *Pagination* for `/matches`, `/fetch`, `/odds` via stateless callback encoding (e.g., `f:p:2`, `m:tp:3`, `m:t:EPL:p:2`). New config `TG_PAGE_SIZE: int = 20`. Helpers: `_paginate()`, `_build_button_grid()` (2-col when labels ≤ 18 chars, 1-col otherwise), `_nav_row()` (◀ Prev / X / Y / Next ▶ with `nop` callback for the indicator and clamped boundaries). New `_cb_nop` no-op handler.
- *`/matches` first layer* now shows full English tournament names with code fallback (Enhancement 3). Groups by tournament code; first non-empty `name_en` wins per code (handles HKJC's same-code-across-seasons quirk). Buttons sorted alphabetically by name.
- *`/jobs` grouping* by `rule_name` with home/away team names. Schema additions to `scheduled_jobs`: `rule_name`, `home_team`, `away_team` persisted at insert time. Legacy docs (pre-Round 2) fall back to the `(legacy)` group and look up team names from `matches_current` via `_enrich_jobs_with_teams()` (bulk `$in` query).
- *Scheduler signature change*: `_schedule_observation(match, obs, now, rule)` — `rule` now threaded from `run_discovery` so the rule name can be persisted with each job.
- *Doc reconciliation*: `docs/commands.md` updated for the Round 1 resilience-hardening commit (notify_error truncation 200→1000, exception type, ValidationError field paths) plus new env vars (`HKJC_REQUEST_TIMEOUT_SECONDS`, `HKJC_MAX_RETRIES`, `HKJC_RETRY_BACKOFF_SECONDS`, `TG_PAGE_SIZE`). `docs/database.md` now has a full `scheduled_jobs` section (previously missing) plus a Nullable Fields note for the relaxed `Match`/`FoPool` fields.
- *Tests*: +25 (296 total). New: `TestPaginate` (6), `TestBuildButtonGrid` (4), `TestNavRow` (4), `TestGroupJobsByRule` (2), `TestFormatJobLine` (2), `TestCmdJobsGrouping` (2), `TestGroupMatchesByTournament` (2), `TestPaginatedCommands` (3). Existing scheduler-persistence tests updated to assert the new `rule_name`/`home_team`/`away_team` fields land in the inserted doc.
...
- **Resilience hardening (2026-05-12)**: HKJC API began returning `null` for `featureStartTime`/`featureMatchSequence`, which crashed full discovery cycles. Audited every `str = ""` field; relaxed display/metadata fields to `Optional[str] = None`; kept `Match.frontEndId` strict because it's a DB unique key and scheduler dedup key — relaxing it would silently corrupt downstream lookups. Parser now skips+logs malformed individual matches instead of aborting. Added budget-aware retry on `Timeout`/`ConnectionError` where `max_retries = min(cap, budget // (timeout + backoff))` so retries never spill into the next polling interval (60s interval + 30s timeout → 1 retry, exactly the user's intent). Retries scoped to transport errors only (no retry on HTTP 4xx or `ValidationError` — neither will change on retry). Raw response (truncated to 2000 chars) now logged at every fetch/parse failure site. `notify_error()` includes exception type and `ValidationError` field-path details. New settings: `HKJC_REQUEST_TIMEOUT_SECONDS`, `HKJC_MAX_RETRIES`, `HKJC_RETRY_BACKOFF_SECONDS`.
- **Round 2 TG UX (2026-05-12)**: Three UX fixes after live use. (1) `/matches`, `/fetch`, `/odds` were silently capping output at 10–15 buttons; replaced with paginated lists using stateless callback encoding (`m:tp:N`, `m:t:CODE:p:N`, `f:p:N`, `o:p:N`) so the handler stays stateless across pages. Page size configurable via `TG_PAGE_SIZE` (default 20). Auto 2-col layout when labels fit in 18 chars, else 1-col. (2) `/jobs` printed only `FB####` — added home/away team names and grouped output by `rule_name`. New schema fields persisted at insert time; legacy docs fall under `(legacy)` and team names get looked up at read time via bulk `$in` query. (3) `/matches` first layer now shows full English tournament names (e.g., "English Premier League (8)") with code fallback. Tournament-name picking: first non-empty `name_en` per code wins, because HKJC reuses the same code across seasons with different `id`s. Out-of-scope: backfill of legacy `scheduled_jobs` docs — read-time fallback is enough; we can do a one-shot migration later if it becomes a nuisance.
